...
function gameOver() {
  drawOrder[9 - lives]();
  modalText.innerHTML =
    `당신은 졌습니다..ㅠ.ㅠ<br>정답 : <span class='answer'>${word}</span>
    <div class='btn-wrap'><button type='button' class='btn reset' onclick='resetGame()'>다시하기</button>
    <button type='button' class='btn reset' onclick='resetGame()'>그만하기</button></div>`;
...
function gameClear() {
  modalText.innerHTML  = 
    `당신은 이겼습니다!!! <br>정답 : <span class='answer'>${word}</span>
    <div class='btn-wrap'><button type='button' class='btn reset' onclick='resetGame()'>다시하기</button>
    <button type='button' class='btn reset' onclick='resetGame()'>그만하기</button></div>`;
  modalClose.style.display = "none";
  modal();
  ...
  letterBtn.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if ([...word].indexOf(guessLetter) != -1) {
        ...
        // false
        liveText.innerHTML = `남은 기회 : <strong>${(lives -= 1)}</strong>번`;
...
function handleKeyboardInput(event) {
  const guessLetter = event.key.toLowerCase(); // Get the pressed key and convert to lowercase

  // Check if the pressed key is a valid alphabet letter
  if (alphabet.includes(guessLetter)) {
    const btn = [...document.querySelectorAll(".buttons button")].find(button => button.innerHTML === guessLetter);
...
function play() {
  ...
  liveText.innerHTML = `남은 기회: <strong>${lives}</strong>번 `;
  createButtons();
  createBlank();
  setCanvas();
  check();
