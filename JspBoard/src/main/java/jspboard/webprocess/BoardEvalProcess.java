...
public class BoardEvalProcess implements WebProcess{
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		String pick = request.getParameter("pick").equals("g") ? 
				"board_good_count" : "board_bad_count";
		
		// 지금 웹브라우저로 접속한 클라이언트가 추천을 한 적이 있으면 추천을 안 해야 한다 
		// (쿠키 또는 세션을 사용)
		...
		// 추천수를 먼저 반영한 후 글 내용을 눌러서 포워드 
		// (대신 글보기 페이지로 리다이렉트)
		String sql1 = String.format("UPDATE jspboard SET %s=%s+1 WHERE board_id = ?", pick, pick);
		
		// String sql2 = "SELECT * FROM jspboard WHERE board_id=?";
...
//			}
			...
			// 이 클라이언트가 추천 또는 비추천했던 글 번호를 모두 목록에 저장 
