...
public class BoardWriteProcess implements WebProcess {
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		String sql = "INSERT INTO jspboard ("
				+ "board_id, board_title, board_password, board_writer, "
				+ "board_writer_ip_addr, board_content) VALUES("
				+ "jspboard_id_seq.nextval, ?, ?, ?, ?, ?)";
		
		// 서버 초기화 시 만들어놓은 연결을 꺼내온다
		try (
			...
			pstmt.setString(2, to_write.getBoard_password());
			pstmt.setString(3, to_write.getBoard_writer());
