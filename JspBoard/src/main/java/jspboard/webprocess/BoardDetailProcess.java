...
public class BoardDetailProcess implements WebProcess{
	...
	public String process(HttpServletRequest request, HttpServletResponse response) {
		...
		String updateSql = "UPDATE jspboard SET board_view_count=board_view_count+1"
				+ " WHERE board_id=?";
		...
		try (
			...
			// 쿠키를 꺼낸 값에 글 id가 포함되어 있다면 조회수가 증가하지 않아야 한다 
			// (update 쿼리문을 스킵한다)
			...
			// 조회수가 올라간 적이 있다 (글 번호가 이미 올라가 있다 -> 추가 안함)
			...
			response.addCookie(cookie); // 응답에 쿠키 실어보내기
